...
if __name__ == '__main__':
    ...
    if args.rcd_file is None:
        args.rcd_file = 'refer_ground_gen_' + os.path.basename(args.radiopedia_json).replace('.json', '')
    ...
    from dataset.test_dataset_refer_grounding import Refer_Grounding_Dataset, Refer_Grounding_Collator
    from torch.utils.data import DataLoader
    from torch.utils.data.distributed import DistributedSampler

    test_dataset = Refer_Grounding_Dataset(
    ...
    collator = Refer_Grounding_Collator(processor)
    ...
    from evaluator.refer_grounding_gen_evaluation import evaluate_json
    ...
    for batch in tqdm.tqdm(dataloader,
        ...
        sample_id = batch['sample_id']  # '16567_4_1_64/0/pos'
        ...
        for i in range(len(sample_id)):
            ...
            results_of_samples.append([dataset_name[i], sample_id[i], [scores], bs_label_ls[i], bs_description_ls[i], bs_type_ls_ls[i], output_texts[i]])
            ...
            if args.visualization:
                ...
                jpg_suffix = sample_id[i].replace('/', '_')
                
                plt.tight_layout()
                plt.savefig(f'{visual_dir}/({round(scores["dice"], 2)}){dataset_name[i]}_{jpg_suffix}.jpg')
                ...
                plt.axis('off')
                plt.tight_layout()
                plt.savefig(f'{prediction_mask_dir}/({round(scores["dice"], 2)}){dataset_name[i]}_{jpg_suffix}.jpg')
                plt.close()
                ...
                plt.axis('off')
                plt.tight_layout()
                plt.savefig(f'{input_image_dir}/({round(scores["dice"], 2)}){dataset_name[i]}_{jpg_suffix}.jpg')
                plt.close()
    ...
    if is_main_process():
        ...
        for dataset_name, sample_id, scores, label_ls, description_ls, type_ls_ls, output_text in results_of_samples:
            if f'{dataset_name}/{sample_id}' not in unique_set:
                unique_set.add(f'{dataset_name}/{sample_id}')
                deduplicated_results_of_samples.append([dataset_name, sample_id, scores, label_ls, description_ls, type_ls_ls, output_text])
        results_of_samples = deduplicated_results_of_samples
        
        # Evaluate the generation results
        
        id2pred = {sample_id:output_text for dataset_name, sample_id, scores, label_ls, description_ls, type_ls_ls, output_text in results_of_samples}

        with open(args.radiopedia_json, 'r') as f:
            source_data = json.load(f)
            
        for datum_idx, datum in source_data.items():
            for group_id, group in datum['abnormality_group'].items() :
                if f'{datum_idx}/{group_id}/pos' in id2pred:
                    group[0]['pos_refer_generation_results'] = id2pred[f'{datum_idx}/{group_id}/pos']
            if f'{datum_idx}/0/neg' in id2pred:
                datum['neg_refer_generation_results'] = id2pred[f'{datum_idx}/0/neg']
        ...
        evaluate_json(
            data_json=f'{args.rcd_dir}/(detailed){args.rcd_file}.json', 
            detailed_output_json=f'{args.rcd_dir}/(detailed){args.rcd_file}.json', 
            merged_output_json=f'{args.rcd_dir}/(merged){args.rcd_file}.json'
        )
            
        # Evaluate the grounding results
        
        results_of_samples = [[dataset_name, sample_id, scores, label_ls, description_ls, type_ls_ls] for dataset_name, sample_id, scores, label_ls, description_ls, type_ls_ls, output_text in results_of_samples]
