...
class Grounding_Gen_Two_Round_Dataset(Dataset):
    ...
    def __init__(self, 
        ...
        # TODO This can be simplied since we dont need to provide seg_image and seg_gt
    ...
    def _get_radiopedia_item(self, idx):
        ...
        if 'abnormality' not in datum:
            answer = "I don't see any abnormalities on the image."
        ...
        # an unhealthy sample    
            ...
            for abnormality in datum['abnormality']:
                ...
                gt_answer.append(random.choice(candidate_gt))
        ...
        return pil_img, None, pil_img_w_prmpt, None, answer, datum['image']
    ...
    def _generate_instruction(self):
        ...
        instruction = f"{prefix} {verb} {target} {condition} and describe the findings{suffix}"
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            try:
                # 随机抽取一个sample
                chosen = random.choices(['deeplesion', 'radiopedia', 'Others'], weights=self.data_source_weight, k=1)[0]
                
                if chosen == 'deeplesion':
                    sample_idx = random.choice(self.deeplesion_idx_ls)
                    pil_img, img, pil_img_w_prmpt, msk, llm_output, img_path = self._get_deeplesion_item(sample_idx)

                elif chosen == 'radiopedia':
                    sample_idx = random.choice(self.radiopedia_idx_ls)
                    pil_img, img, pil_img_w_prmpt, msk, llm_output, img_path = self._get_radiopedia_item(sample_idx)
                    
                else:
                    sample_idx = random.choice(self.other_data_idx_ls)
                    pil_img, img, pil_img_w_prmpt, msk, llm_output, img_path = self._get_other_data_item(sample_idx)
        ...
        return {
            'task_type': 'grounding_gen_two_round',
            'vlm_data': format_two_round_data(pil_img, instruction, pil_img_w_prmpt, llm_output),
            # 'seg_img': img,
            # 'seg_gt': msk,
            'img_path': img_path
            }
...
class Grounding_Gen_One_Round_Dataset(Dataset):
    def __init__(self, 
                 # Seg Data
                 deeplesion_json=None,
                 radiopedia_json=None,
    ...
    def _get_radiopedia_item(self, idx):
        ...
        return pil_img, img, None, msk, None, datum['image']
    ...
    def __getitem__(self, idx):
        ...
        return {
            'task_type': 'grounding_gen_one_round',
...
if __name__ == '__main__':
    ...
    # Test the dataset
    dataset = Grounding_Gen_Two_Round_Dataset(
        radiopedia_json='/mnt/hwfile/medai/zhaoziheng/Med_ULS/Radiopedia_CT/data_jsonl/axial_jsonl/merged_train(8161_axial).json',
    )
