...
class Qwen2_5_VLForConditionalGeneration_wSEG(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    config_class = Qwen2_5_VLConfig
    _no_split_modules = ["Qwen2_5_VLDecoderLayer", "Qwen2_5_VLVisionBlock"]
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        pixel_values: Optional[torch.Tensor] = None,
        segmentation_image=None, # NOTE: Seg Input Image (b d h w)
        segmentation_gt_mask=None, # NOTE: Seg GT (b n h w)
        pixel_values_videos: Optional[torch.FloatTensor] = None,
        image_grid_thw: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        ...
        # WARNING generate segmentations and calculate dice loss

        if labels is not None:
            
            seg_token_mask = (labels == self.seg_token_idx) # B, seq_length
            try:
            ...
            if n_seg_tokens > 0:
                ...
                # Padding
                seg_token_counts = seg_token_mask.sum(dim=1)  # Count of SEG tokens per sample
                max_seg_tokens = seg_token_counts.max().item()
                padded_seg_latent_embeddings = torch.zeros(
                    (batch_size, max_seg_tokens, hidden_dim), 
                    dtype=hidden_states.dtype, 
                    device=hidden_states.device
                )
                query_mask = torch.zeros(
                    (batch_size, max_seg_tokens), 
                    dtype=torch.bool, 
                    device=hidden_states.device
                )
                
                # Fill in actual embeddings and mask (sample by sample)
                for i in range(batch_size):
                    # Get indices where SEG tokens appear in this sample
                    sample_indices = torch.where(seg_token_mask[i])[0]
                    sample_seg_count = len(sample_indices)
                    
                    if sample_seg_count > 0:
                        # Extract embeddings directly using indices
                        sample_embeddings = hidden_states[i, sample_indices]
                        padded_seg_latent_embeddings[i, :sample_seg_count] = sample_embeddings
                        query_mask[i, :sample_seg_count] = 1
            ...
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                pred_mask = self.seg_model(segmentation_image, padded_seg_latent_embeddings) # (B, max_seg_tokens, H, W)
                
            seg_loss, unreduced_batch_dice_loss, unreduced_batch_ce_loss = self.seg_loss(pred_mask, segmentation_gt_mask, query_mask)
            loss += seg_loss
            self.dice_loss_m.update(torch.mean(unreduced_batch_dice_loss).item(), 1)
            self.dice_loss_m.update(torch.mean(unreduced_batch_ce_loss).item(), 1)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # WARNING This is necessary to pass seg parameters in
        for k in ['segmentation_gt_mask', 'segmentation_image']:
            if k not in kwargs:
                continue
...
__all__ = ["Qwen2_5_VLForConditionalGeneration", "Qwen2_5_VLForConditionalGeneration_wSEG", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel"]
