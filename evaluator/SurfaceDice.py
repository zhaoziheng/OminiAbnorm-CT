...
def compute_sensitivity(gt, seg):
    """计算敏感度 (True Positive Rate)"""
    # 如果没有GT pos，则Pred必须没有pos
    if np.sum(gt) == 0:
      if np.sum(seg) == 0.0:
        return 1.0
      else:
    ...
    # 惩罚Pred全是pos（OverSeg）
    if np.sum(gt == 0) != 0 and np.sum(seg == 0) == 0:
      return 0.0
      
    return np.sum(gt & seg) / np.sum(gt)  # 如果存在正例，seg不存在正例子，则se=0

def compute_specificity(gt, seg):
    """计算特异度 (True Negative Rate)"""
    # 如果没有GT neg，则Pred必须没有neg
    if np.sum(gt == 0) == 0:
      if np.sum(seg == 0) == 0:
        return 1.0  # 如果没有真实负例
      else:
    ...
    # 惩罚Pred全是neg（UnderSeg）
    if np.sum(gt) > 0 and np.sum(seg) == 0:
        return 0.0
    
    true_negatives = np.sum((gt == 0) & (seg == 0))
    actual_negatives = np.sum(gt == 0)
    return true_negatives / actual_negatives

def compute_detection_ratio(gt, seg, ratio):
    """
    计算检测率 (Detection Ratio)
    一个病灶被认为正确检测到，如果在连通域内至少有ratio比例体素被准确预测
    """
    ...
    if np.sum(gt) == 0:
        ...
        if np.sum(seg) == 0:
            return 1.0
        # 如果预测有病灶，则认为是错误检测(过度检测)
        else:
            return 0.0
    
    # 标记真实标签中的所有连通区域
    labeled_gt, num_lesions = ndimage.label(gt)
    ...
    for i in range(1, num_lesions + 1):
        ...
        # 如果在该病灶区域内至少有一个体素被正确预测，则认为该病灶被检测到
        # 计算在该病灶区域内被正确预测的体素比例
        lesion_size = np.sum(lesion_mask)
        correctly_predicted_voxels = np.sum(seg & lesion_mask)
        detection_fraction = correctly_predicted_voxels / lesion_size
        
        # 如果在该病灶区域内至少有ratio比例的体素被正确预测，则认为该病灶被检测到
        if detection_fraction >= ratio:
            detected_lesions += 1
    
    # 计算检测率
    return detected_lesions / num_lesions