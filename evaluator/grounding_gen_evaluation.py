...
def evaluate_json(data_json, 
    ...
    for key, datum in tqdm(data.items()):
        ...
        if 'grounding_generation_gt' not in datum:
            continue
        else:
            gt = datum['grounding_generation_gt']

        if 'grounding_generation_results' not in datum:
            answer = ''
        else:
            answer = datum['grounding_generation_results']
                
        try:                    
            if calculate_bertscore:
                bertscore = bertscore_model.compute(predictions=[answer], references=[gt], lang='en')['f1']
                bertscore = sum(bertscore)/len(bertscore) * 100
                datum['grounding_generation_bertscore'] = bertscore
            else:
                bertscore = 0
            
            if calculate_meteor:
                meteor = meteor_model.compute(predictions=[answer], references=[gt])['meteor'] * 100
                datum['grounding_generation_meteor'] = meteor
            else:
                meteor = 0
            ...
            if calculate_radgraph:
                ...
                datum['grounding_generation_rouge1'] = rouge1
                datum['grounding_generation_rougeL'] = rougeL
            else:
                rouge1 = rougeL = 0
            ...
            if calculate_radgraph:
                ...
                datum['grounding_generation_radgraph'] = radgraph_score
            else:
                radgraph_score = 0
            ...
            if calculate_bleu:
                ...
                datum['grounding_generation_blue1'] = bleu1_score
                datum['grounding_generation_blue2'] = bleu2_score
                datum['grounding_generation_blue3'] = bleu3_score
                datum['grounding_generation_blue4'] = bleu4_score
            else:
            ...
            if calculate_ratescore:
                ...
                datum['grounding_generation_ratescore'] = rate_score
            else:
                rate_score = 0
            if rate_score == 50:
                rate_score = 0
